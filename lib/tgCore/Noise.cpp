...
namespace tg
{
    namespace core
    {
        ...
        Noise::Noise()
        {
            ...
            _p.insert(_p.begin(), permutation.begin(), permutation.end());
            _p.insert(_p.end(), permutation.begin(), permutation.end());
        }
        ...
        double Noise::get(double x, double y, double z)
        {
            ...
            const int A  = _p[X] + Y;
            const int AA = _p[A] + Z;
            const int AB = _p[A + 1] + Z;
            const int B  = _p[X + 1] + Y;
            const int BA = _p[B] + Z;
            const int BB = _p[B + 1] + Z;
