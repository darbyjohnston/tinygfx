...
namespace tg
{
    namespace gl
    {
        namespace
        {
            ...
            class BoxPackingNode : public std::enable_shared_from_this<BoxPackingNode>
            {
            private:
                BoxPackingNode(
                    int border,
                    const std::shared_ptr<TimestampManager>&);

            public:
                static std::shared_ptr<BoxPackingNode> create(
                    int border,
                    const std::shared_ptr<TimestampManager>&);
                ...
                std::shared_ptr<BoxPackingNode> insert(const std::shared_ptr<Image>&);
            };
            ...
            int border)
        {
            ...
            for (size_t i = 0; i < p.textureCount; ++i)
            {
                ...
                auto texture = Texture::create(ImageInfo(textureSize, textureSize, textureType), textureOptions);
